...
class Dealer(commands.Bot):
    ...
    async def invoke(self, ctx):
        if await self.is_blacklisted(ctx.author.id, ctx.command.name):
...
class UtilsCog(commands.Cog):
    ...
    async def blacklist(self, ctx, user: discord.User, command_name: str):
        ...
        await ctx.send(f"🔒 {user.mention} has been blacklisted from using `-{cmd_name}`.")
    ...
    async def whitelist(self, ctx, user: discord.User, command_name: str):
        ...
        if user.id in self.bot.blacklist and cmd_name in self.bot.blacklist[user.id]:
            ...
            await ctx.send(f"✅ {user.mention} is now whitelisted for `-{cmd_name}`.")
        else:
            await ctx.send(f"ℹ️ {user.mention} was not blacklisted from `-{cmd_name}`.")
...
- `-dice <1-6> <amount>`: Bet on a dice roll.
...
- `-bl @user <command_name>`: Blacklist a user for a command.
- `-wl @user <command_name>`: Whitelist a user for a command.
