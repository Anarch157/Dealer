...
DAILY_GAME_CHANCES = 10
LIMITED_GAMES = {"coin", "dice", "roulette", "slots", "blackjack", "rdb", "tossbattle"}

def current_play_date():
    return date.today().isoformat()

def game_chance_footer_text(used_count: int, extra_text: str = None) -> str:
    base = f"🎯 Chances used: {used_count}/{DAILY_GAME_CHANCES}"
...
def set_game_chance_footer(embed: discord.Embed, source, extra_text: str = None):
    if isinstance(source, dict):
        ...
        used_count = get_interaction_chances_used(source)

    if used_count is not None:
        embed.set_footer(text=game_chance_footer_text(used_count, extra_text))
    elif extra_text:
        embed.set_footer(text=extra_text)
    return embed
...
def cooldown(cooldown_seconds: int):
    def decorator(func):
        ...
        async def wrapper(interaction: discord.Interaction, *args, **kwargs):
            ...
            cooldown_end = await bot.get_command_cooldown_end(user_id, command_name)
            if cooldown_end > now:
            ...
            await bot.set_command_cooldown(user_id, command_name, now + cooldown_seconds)
            ...
            if bet_amount > MAX_BET_AMOUNT:
                await interaction.response.send_message(
                    embed=discord.Embed(
                        ...
                        description=f"The maximum bet is {MAX_BET_AMOUNT:,} coins.",
                        color=discord.Color.gold()
                    ),
                    ephemeral=True
                )
...
# -------------------------------------------------- Decorator: Daily Game Limit --------------------------------------------------

def daily_game_limit(game_name: str):
    def decorator(func):
        @functools.wraps(func)
        async def wrapper(interaction: discord.Interaction, *args, **kwargs):
            bot = interaction.client
            allowed, used = await bot.consume_game_chance(interaction.user.id, game_name)
            bot.game_chance_context[id(interaction)] = used
            if not allowed:
                await interaction.response.send_message(
                    embed=discord.Embed(
                        title="⏳ Daily game limit reached!",
                        description=f"You have already used all {DAILY_GAME_CHANCES} chances for `{game_name}` today.",
                        color=discord.Color.gold()
                    ),
                    ephemeral=True
                )
...
class Dealer(discord.Client):
    ...
    async def on_message(self, message):
        ...
        game = self.active_games.get(message.author.id)
        if not game:
            return
        ...
        if game["type"] == "blackjack":
            if content not in {"hit", "stand"}:
                return

            wallet, bank = await self.get_balances(message.author.id)
            ...
            if content == "hit":
                ...
                if player_value > 21:
                    wallet -= bet_amount
                    await self.update_balances(message.author.id, wallet, bank)
                    clear_active_game(message.author.id)

                    embed = discord.Embed(
                        title=f"❌ {message.author.display_name} busted in Blackjack!",
                        color=discord.Color.red()
                    )
                    embed.add_field(name="🃏 Your Hand", value=f"{format_cards(player_hand)} ({player_value})", inline=False)
                    embed.add_field(name="🃏 Dealer Hand", value=f"{format_cards(dealer_hand)} ({blackjack_hand_value(dealer_hand)})", inline=False)
                    embed.add_field(name="💸 Lost", value=f"{bet_amount:,} coins", inline=False)
                    embed.add_field(name="💰 Wallet", value=f"{wallet:,}", inline=False)
                    set_game_chance_footer(embed, game)
                    await message.channel.send(embed=embed)
                    return

                embed = discord.Embed(
                    title="🃏 Blackjack",
                    description="Reply with **hit** or **stand**.",
                    color=discord.Color.gold()
                )
                embed.add_field(name="🃏 Your Hand", value=f"{format_cards(player_hand)} ({player_value})", inline=False)
                embed.add_field(name="🃏 Dealer Shows", value=format_card(dealer_hand[0]), inline=False)
                set_game_chance_footer(embed, game)
                await message.channel.send(embed=embed)
                return
            ...
            if dealer_value > 21 or player_value > dealer_value:
                winnings = int(bet_amount * 0.9)
                wallet += winnings
                title = f"🏆 {message.author.display_name} won in Blackjack!"
                description = f"You won {winnings:,} coins."
                color = discord.Color.green()
            elif player_value < dealer_value:
                wallet -= bet_amount
                title = f"❌ {message.author.display_name} lost in Blackjack!"
                description = f"You lost {bet_amount:,} coins."
                color = discord.Color.red()
            else:
                title = f"🤝 {message.author.display_name} pushed in Blackjack!"
                description = "It's a tie. No coins were won or lost."
                color = discord.Color.gold()
            ...
            embed.add_field(name="🃏 Your Hand", value=f"{format_cards(player_hand)} ({player_value})", inline=False)
            embed.add_field(name="🃏 Dealer Hand", value=f"{format_cards(dealer_hand)} ({dealer_value})", inline=False)
            embed.add_field(name="💰 Wallet", value=f"{wallet:,}", inline=False)
            set_game_chance_footer(embed, game)
            await message.channel.send(embed=embed)
            return
        ...
        if game["type"] == "rdb":
            ...
            def result_embed(step_title, description, cards_value, reward=None, lost=None):
                color = discord.Color.green() if reward is not None else discord.Color.red()
                embed = discord.Embed(title=step_title, description=description, color=color)
                if cards_value:
                    embed.add_field(name="🃏 Cards", value=cards_value, inline=False)
                if reward is not None:
                    embed.add_field(name="💰 Won", value=f"{reward:,} coins", inline=False)
                if lost is not None:
                    embed.add_field(name="💸 Lost", value=f"{lost:,} coins", inline=False)
                embed.add_field(name="💰 Wallet", value=f"{wallet:,}", inline=False)
                set_game_chance_footer(embed, game)
                return embed

            if step == 1:
                if content not in {"red", "black"}:
                    return
                card1 = deck.pop()
                cards.append(card1)
                if rdb_card_color(card1) != content:
                    wallet -= bet_amount
                    await self.update_balances(message.author.id, wallet, bank)
                    clear_active_game(message.author.id)
                    await message.channel.send(embed=result_embed(
                        "❌ Ride the Bus - Lost on Step 1",
                        f"You guessed **{content}**. The first card was **{format_card(card1)}**.",
                        format_cards(cards),
                        lost=bet_amount
                    ))
                    return
                ...
                game["step"] = 2
                embed = discord.Embed(
                    title="✅ Ride the Bus - Step 1 Cleared",
                    description=(
                        f"First card: **{format_card(card1)}**\n"
                        "Step 2: reply with **higher** or **lower**."
                    ),
                    color=discord.Color.green()
                )
                embed.add_field(name="🃏 Cards", value=format_cards(cards), inline=False)
                set_game_chance_footer(embed, game)
                await message.channel.send(embed=embed)
                return
            ...
            if step == 2:
                ...
                card2 = deck.pop()
                cards.append(card2)
                v1 = rdb_card_value(cards[0])
                v2 = rdb_card_value(card2)
                correct = v2 > v1 if content == "higher" else v2 < v1
                if not correct:
                    clear_active_game(message.author.id)
                    await message.channel.send(embed=result_embed(
                        "❌ Ride the Bus - Lost on Step 2",
                        f"You guessed **{content}**. The second card was **{format_card(card2)}**. No coins were won or lost.",
                        format_cards(cards)
                    ))
                    return
                ...
                game["step"] = 3
                embed = discord.Embed(
                    title="✅ Ride the Bus - Step 2 Cleared",
                    description=(
                        f"Cards so far: **{format_cards(cards)}**\n"
                        "Step 3: reply with **inside** or **outside**."
                    ),
                    color=discord.Color.green()
                )
                embed.add_field(name="🃏 Cards", value=format_cards(cards), inline=False)
            ...
            if step == 3:
                if content not in {"inside", "outside"}:
                    return
                card3 = deck.pop()
                cards.append(card3)
                v1 = rdb_card_value(cards[0])
                v2 = rdb_card_value(cards[1])
                v3 = rdb_card_value(card3)
                low = min(v1, v2)
                high = max(v1, v2)
                inside = low < v3 < high
                correct = inside if content == "inside" else not inside
                if not correct:
                    clear_active_game(message.author.id)
                    await message.channel.send(embed=result_embed(
                        "❌ Ride the Bus - Lost on Step 3",
                        f"You guessed **{content}**. The third card was **{format_card(card3)}**. No coins were won or lost.",
                        format_cards(cards)
                    ))
                    return
                ...
                game["step"] = 4
                embed = discord.Embed(
                    title="✅ Ride the Bus - Step 3 Cleared",
                    description=(
                        f"Cards so far: **{format_cards(cards)}**\n"
                        "Step 4: reply with **spades**, **hearts**, **diamonds**, or **clubs**."
                    ),
                    color=discord.Color.green()
                )
                embed.add_field(name="🃏 Cards", value=format_cards(cards), inline=False)
            ...
            if step == 4:
                if content not in suit_words:
                    return
                card4 = deck.pop()
                cards.append(card4)
                if card4[1] != suit_words[content]:
                    clear_active_game(message.author.id)
                    await message.channel.send(embed=result_embed(
                        "❌ Ride the Bus - Lost on Step 4",
                        f"You guessed **{content}**. The final card was **{format_card(card4)}**. No coins were won or lost.",
                        format_cards(cards)
                    ))
                    return

                winnings = int(bet_amount * 4)
                wallet += winnings
                await self.update_balances(message.author.id, wallet, bank)
                clear_active_game(message.author.id)

                embed = discord.Embed(
                    title=f"🏆 {message.author.display_name} completed Ride the Bus!",
                    description=f"You won {winnings:,} coins.",
                    color=discord.Color.green()
                )
                embed.add_field(name="🃏 Cards", value=format_cards(cards), inline=False)
...
# -------------------------------------------------- Function: Command Cooldowns --------------------------------------------------

    async def get_command_cooldown_end(self, user_id, command_name):
        now = time.time()
        async with self.db.execute(
            'SELECT cooldown_end FROM command_cooldowns WHERE user_id = ? AND command_name = ?',
        ...
        if cooldown_end <= now:
            await self.db.execute(
                'DELETE FROM command_cooldowns WHERE user_id = ? AND command_name = ?',
                (user_id, command_name)
            )
            await self.db.commit()
            return 0
    ...
    async def set_command_cooldown(self, user_id, command_name, cooldown_end):
        await self.db.execute(
            '''
            ...
            ''',
            (user_id, command_name, float(cooldown_end))
        )
        await self.db.commit()

    async def clear_command_cooldowns(self):
        await self.db.execute('DELETE FROM command_cooldowns')
        await self.db.commit()
...
# -------------------------------------------------- Function: Game Chances --------------------------------------------------

    async def get_game_chances_used(self, user_id: int, game_name: str):
        play_date = current_play_date()
        async with self.db.execute(
            'SELECT used_count FROM game_chances WHERE user_id = ? AND game_name = ? AND play_date = ?',
            (user_id, game_name, play_date)
        ) as cursor:
            row = await cursor.fetchone()
            return row[0] if row else 0

    async def consume_game_chance(self, user_id: int, game_name: str):
        play_date = current_play_date()
        used = await self.get_game_chances_used(user_id, game_name)
        if used >= DAILY_GAME_CHANCES:
            return False, used

        await self.db.execute(
            """
            INSERT INTO game_chances (user_id, game_name, play_date, used_count)
            VALUES (?, ?, ?, 1)
...
async def coin(interaction: discord.Interaction, bet_option: str, bet_amount: int):
    ...
    set_game_chance_footer(embed, interaction)
    await interaction.response.send_message(embed=embed)
...
async def roulette(interaction: discord.Interaction, bet_option: str, bet_amount: int):
    ...
    if (isinstance(bet_option, int) and bet_option == result) or (isinstance(bet_option, str) and bet_option_lower == str(result)):
        won = True
        payout = int(bet_amount * 34.15)
    elif bet_option_lower == "high" and 19 <= result <= 36:
        won = True
        payout = int(bet_amount * 0.953)
...
async def slots(interaction: discord.Interaction, bet_amount: int):
    ...
    if spin[0] == spin[1] == spin[2]:
        multiplier = 4.75   # Jackpot, tuned for ~95% RTP with 5 symbols
...
async def tossbattle(interaction: discord.Interaction, user: discord.Member, amount: int):
    ...
    chance_footer = game_chance_footer_text(get_interaction_chances_used(interaction) or 0)
    ...
    payout = int(amount * 1.9)
    await bot.update_balances(winner.id, winner_wallet + payout, winner_bank)

    result_embed = discord.Embed(
        title="🪙 Toss Battle Result",
        description=f"Coin toss result: **{result.upper()}**!\n🎉 {winner.mention} wins {payout:,} coins!",
        color=discord.Color.green()
    )
    set_game_chance_footer(result_embed, interaction)
...
@bot.tree.command(name="blackjack", description="Start a text blackjack game against the dealer")
@app_commands.describe(bet_amount="How many coins to bet")
@blacklist_check()
@requires_coins(param_name="bet_amount")
@daily_game_limit("blackjack")
async def blackjack(interaction: discord.Interaction, bet_amount: int):
    if interaction.user.id in bot.active_games:
        await interaction.response.send_message(
            "You already have an active game. Finish it before starting another one.",
            ephemeral=True
        )
        return
    ...
    player_value = blackjack_hand_value(player_hand)
    dealer_visible = format_card(dealer_hand[0])

    bot.active_games[interaction.user.id] = {
        "type": "blackjack",
    ...
    if player_value == 21:
        wallet, bank = await bot.get_balances(interaction.user.id)
        winnings = int(bet_amount * 1.35)
        wallet += winnings
        await bot.update_balances(interaction.user.id, wallet, bank)
        clear_active_game(interaction.user.id)

        embed = discord.Embed(
            title=f"🏆 {interaction.user.display_name} hit Blackjack!",
            description=f"You won {winnings:,} coins.",
            color=discord.Color.green()
        )
        embed.add_field(name="🃏 Your Hand", value=f"{format_cards(player_hand)} ({player_value})", inline=False)
    ...
    embed = discord.Embed(
        ...
        description="Reply with **hit** or **stand** in this channel.",
        color=discord.Color.gold()
    )
    embed.add_field(name="🃏 Your Hand", value=f"{format_cards(player_hand)} ({player_value})", inline=False)
    embed.add_field(name="🃏 Dealer Shows", value=dealer_visible, inline=False)
...
# -------------------------------------------------- Command: Ride The Bus --------------------------------------------------

@bot.tree.command(name="rdb", description="Start a text Ride the Bus game")
@app_commands.describe(bet_amount="How many coins to bet")
@blacklist_check()
@requires_coins(param_name="bet_amount")
@daily_game_limit("rdb")
async def rdb(interaction: discord.Interaction, bet_amount: int):
    if interaction.user.id in bot.active_games:
        await interaction.response.send_message(
            "You already have an active game. Finish it before starting another one.",
            ephemeral=True
    ...
    embed = discord.Embed(
        title="🚌 Ride the Bus Started",
        description=(
            "Step 1: guess the color of the first card.\n"
            "Reply with **red** or **black** in this channel."
        ),
        color=discord.Color.gold()
    )
    embed.add_field(name="🎯 Bet Amount", value=f"{bet_amount:,}", inline=False)
...
@bot.tree.command(name="resetlimits", description="Reset all game chances and cooldowns (owner only)")
@owner_only()
async def resetlimits(interaction: discord.Interaction):
    await bot.db.execute('DELETE FROM game_chances')
    await bot.clear_command_cooldowns()

    if hasattr(bot, "game_chance_context"):
        bot.game_chance_context.clear()

    embed = discord.Embed(
        title="♻️ Limits Reset",
        description="All daily game chances and command cooldowns have been reset.",
        color=discord.Color.green()
    )
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def help_command(interaction: discord.Interaction):
    ...
    description = (
        ...
        f"`/blackjack <amount>` ― Start blackjack, then reply hit/stand\n"
        f"`/rdb <amount>` ― Start Ride the Bus, then reply with guesses\n"
        f"`/fortune` ― Try your luck (hourly)\n"
        f"`/top` ― Richest users\n"
        f"`/quiz <category>` ― Answer trivia\n"
        f"`/tossbattle @user <amount>` ― Toss battle\n"
        f"Limits: 10 chances per game per day, max bet {MAX_BET_AMOUNT:,} coins.\n\n"
        f"**🎵 Music**\n"
        ...
        f"`/resetlimits` ― Reset chances/cooldowns\n"
...
# -------------------------------------------------- Run Bot --------------------------------------------------
if bot.token == "no_token":
    print("[-] No token for bot specified in the config.txt file")
else:
    bot.run(bot.token)
    if bot.debug_channel == 0:
        print("[-] No debug channel id specified in the config.txt file")
    if bot.source_code_link == "https://www.youtube.com/watch?v=dQw4w9WgXcQ":
        print("[-] No source code link specified in the config.txt file. This is a clear violation of GNU AFFERO GENERAL PUBLIC LICENSE")
